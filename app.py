...
def results():
    ...
    if geo_resp.get('status') != 'OK' or not geo_resp.get('results'):
        return render_template('results.html', error='Invalid location', places=[], center_json=json.dumps({'lat':0,'lng':0}), location_text=location, keyword=keyword, browser_key=GOOGLE_BROWSER_KEY)
